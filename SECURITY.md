...
| 6 | Human (social-engineering / coercion resistance) | **Operator responsibility** | No consensus-level "duress password" or plausible-deniability wallet exists in `src/wallet_cli.rs` today. This is a real, buildable wallet-UX feature (e.g. a decoy passphrase revealing a low-balance wallet) but it does not exist yet. |
